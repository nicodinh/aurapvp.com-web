			...
			if ($result = mysqli_query($link4, $query)){
				...
				if ($result = mysqli_query($link4, $query)){
					...
					if ($result = mysqli_query($link2, $query)){
						$row = mysqli_fetch_assoc($result);
						mysqli_free_result($result);
						if ($rowmcMMo["excavation"] >= 400 and $rowmcMMo["fishing"] >= 400 and $rowmcMMo["herbalism"] >= 400 and $rowmcMMo["mining"] >= 400 and $rowmcMMo["woodcutting"] >= 400 and $rowmcMMo["axes"] >= 400 and $rowmcMMo["archery"] >= 400 and $rowmcMMo["swords"] >= 400 and $rowmcMMo["taming"] >= 400 and $rowmcMMo["unarmed"] >= 400 and $row["hfExplorateur"] == 0){
